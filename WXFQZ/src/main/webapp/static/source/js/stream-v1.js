	...
	Main.prototype = {
		...
		upload : function(index) {
			...
			xhr.onreadystatechange = function() {
			    ...
			    var token, server,fileJson;
				try {
					try {
						token = eval("(" + xhr.responseText + ")").token;
						server = eval("(" + xhr.responseText + ")").server;
						fileJson = eval("(" + xhr.responseText + ")").fileJson;
					} catch(e) {}
					if (token) {
						$tokeThis.get("onCompleteTk")(fileJson);
