	...
	<div class="row" style="margin-left: 0; margin-right: 0">
		<div class="col-xs-12">
			<div class="header smaller lighter blue">
				<h1>结果概览</h1>
				<a href="${ctxPath}/v1/html/analyse/result/resultList.do"
			...
			<div>
				<ul class="resultView">
					...
					<li class="resultViewTable" style="border: none; background: none">
						<table>
							<tr>
								...
								<td class="rightContent"><div class="danH">${rw.ms}</div></td>
							</tr>
							...
							<tr>
								...
								<td class="rightContent" colspan="3"><fmt:formatDate
										value="${rw.cjsj}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
							</tr>
						</table>
					</li>
					<li style="font-size: 16px; font-weight: bold; line-height: 35px;">
					...
					<li class="resultViewTable"
						style="padding-left: 44px; padding-right: 10px;">
						<table class="danhangT">
							<tr>
								<td class="leftTilte" style="width: 16%; max-width: 115px;">税种：</td>
								<td class="rightContent" style="width: 35%"><div>${rw.sz}</div></td>
								<td class="leftTilte">核算方式：</td>
								<td class="rightContent"><div>${rw.hsfsmc}</div></td>
							</tr>
							<tr>
								<td class="leftTilte">主管税务机关：</td>
								<td class="rightContent"><div>${rw.swjgCjMc}</div></td>
								<td class="leftTilte">登记注册类型：</td>
								<td class="rightContent"><div>${rw.djzclx}</div></td>
							</tr>
							<tr>
								<td class="leftTilte" style="width: 16%; max-width: 120px;">行业：</td>
								<td class="rightContent" style="width: 35%"><div>${rw.hy}</div></td>
								<td class="leftTilte">税款所属期：</td>
								<td class="rightContent"><c:if
										test="${(rw.sjkssj == '' || rw.sjkssj == null) && (rw.sjjssj == '' || rw.sjjssj == null)}">所有 </c:if>
									<c:if
										test="${(rw.sjkssj != null && rw.sjkssj != '') || (rw.sjjssj != null && rw.sjjssj != '')}">
										<fmt:formatDate value="${rw.sjkssj }" pattern="yyyy-MM" />~<fmt:formatDate
											value="${rw.sjjssj }" pattern="yyyy-MM" />
									</c:if></td>
							</tr>
							<tr>
								<td class="leftTilte">企业规模：</td>
								<td class="rightContent" colspan="3"><div>${rw.qygm}</div></td>
							</tr>
						</table>
					</li>
				</ul>
			...
			<div style="padding-left: 25px;">
				<table
					...
					<tbody>
						<tr>
							...
							<td class="center">${docCount1 - docCount2}（<fmt:formatNumber
									value="${(docCount1 - docCount2)/docCount1*100}" pattern="#.##"
									minFractionDigits="2" />%）
							</td>
							<td class="center">${docCount2}（<c:if test="${docCount1==0}">0</c:if>
								<c:if test="${docCount1!=0 }">
									<fmt:formatNumber value="${docCount2/docCount1*100}"
				...
				<table
					...
					<tbody>
						<tr>
							<td class="center"><fmt:formatNumber value="${ycskze}"
									pattern='#,###.##' minFractionDigits="2" /></td>
							<td class="center"><fmt:formatNumber value="${ysbskze}"
									pattern="#,###.##" minFractionDigits="2" /></td>
							<td class="center"><fmt:formatNumber value="${lsje}"
									pattern="#,###.##" minFractionDigits="2" /></td>
							<td class="center"><fmt:formatNumber value="${zlsjezb*100}"
									pattern="#.##" minFractionDigits="2" />%</td>
						</tr>
					</tbody>
				</table>
			...
			<div style="text-align: right">
				<a
					href="${ctxPath}/v1/html/analyse/result/taxpayerList.do?TASKID=${rw.rwh}"
					class="taxBtn">一户式报告</a>
			</div>
			<div class="payMethodTitle">
				<ul id="myTab" class="nav nav-tabs">
					<li class="active"><a href="#highRisk" data-toggle="tab">高风险纳税人TOP
							10</a></li>
					<li <c:if test="${length1 == 1}">class="defaultLi"</c:if>><c:if
							test="${length1 == 1}">
							<a href="javascript:void(0)">税务机关</a>
						</c:if> <c:if test="${length1 != 1}">
							<a href="#shuiwuTab" data-toggle="tab"
								onclick="setTimeout(function(){creatShuiwu()},500)">税务机关</a>
						</c:if></li>
					<li <c:if test="${length2 == 1}">class="defaultLi"</c:if>><c:if
							test="${length2 == 1}">
							<a href="javascript:void(0)">企业类型</a>
						</c:if> <c:if test="${length2 != 1}">
							<a href="#qiyeTab" data-toggle="tab"
								onclick="setTimeout(function(){creatQiye()},500)">企业类型</a>
						</c:if></li>

					<li><a href="#riskLevelTab" data-toggle="tab"
						onclick="setTimeout(function(){creatfengxian()},500)">风险级别</a></li>
				</ul>
			</div>
			<div id="myTabContent" class="tab-content">
				<div class=" childTab  tab-pane fade  in active" id="highRisk">
					<div class="col-xs-12 col-sm-7 " id="highRiskImg"
						style="height: 500px;"></div>
					<div class="col-xs-12 col-sm-5 " style="min-height: 500px;">
						<div style="text-align: right">
							<a
								href="${ctxPath}/v1/html/analyse/result/taxpayerList.do?TASKID=${rw.rwh}"
								class="taxLookDetail">查看详情</a>
						</div>
						<table
							class="resultTable table table-info table-striped table-bordered table-hover">
							<thead>
				...
				<div class=" childTab  tab-pane fade" id="shuiwuTab">
					...
					<div class="col-xs-4" style="min-height: 500px;">
						<div style="text-align: right">
							<a
								href="${ctxPath}/v1/html/analyse/result/detailList.do?type=1&taskId=${rw.rwh}"
								class="taxLookDetail">查看详情</a>
						</div>
						<table
							class="resultTable table table-info table-striped table-bordered table-hover">
				...
				<div class=" childTab  tab-pane fade" id="qiyeTab">
					...
					<div class="col-xs-4" style="min-height: 500px;">
						<div style="text-align: right">
							<a
								href="${ctxPath}/v1/html/analyse/result/detailList.do?type=2&taskId=${rw.rwh}"
								class="taxLookDetail">查看详情</a>
						</div>
						<table
							class="resultTable table table-info table-striped table-bordered table-hover">
				...
				<div class=" childTab  tab-pane fade" id="riskLevelTab">
					...
					<div class="col-xs-4" style="min-height: 500px;">
						<div style="text-align: right">
							<a
								href="${ctxPath}/v1/html/analyse/result/detailList.do?type=3&taskId=${rw.rwh}"
								class="taxLookDetail">查看详情</a>
						</div>
						<table
							class="resultTable table table-info table-striped table-bordered table-hover">
							<thead>
								<tr>
									<th>NO.</th>
									<th>风险级别  <span class="fa fa-cog setRisk" data-toggle="tooltip" title="自定义风险级别"></span>
									</th>
									<th>户数</th>
								</tr>
							</thead>
				...
				<div class="iso_alertBody">
					<form class="form-horizontal">
						<div class="form-group">
							<label
								class="col-sm-2 col-xs-4 control-label no-padding-right existenceCon">定义方式：</label>
							<div class="col-sm-9 col-xs-8">
								<div class="radio radioUl">
									<label> <input value="0" name="dataType" type="radio"
										checked class="ace"> <span class="lbl">数值</span>
									</label> <label> <input value="1" name="dataType" type="radio"
										class="ace"> <span class="lbl">百分比</span>
									</label>
								</div>
							</div>
						</div>
						<div id="riskSetArea">
							<div id="firstSetRisk">
								<div class="form-group">
									<label
										class="col-sm-2 col-xs-4 control-label no-padding-right existenceCon">风险级别名称：</label>
									<div class="col-sm-9 col-xs-8">
										<input type="text" value=""
											class="riskName col-xs-12 col-sm-8"
											onblur="checkInputNull(this)">
									</div>
								</div>
								<div class="form-group">
									<label
										class="col-sm-2 col-xs-4 control-label no-padding-right existenceCon">风险级别区间：</label>
									<div class="col-sm-9 col-xs-8">
										<input type="number" value="0"
											class="riskMinCount col-xs-12 col-sm-4" disabled><span
											class="riskUnit"></span><span class="qujianline">-</span> <input
											type="number" value=""
											class="riskMaxCount lastRiskLevel col-xs-12 col-sm-4"
											onblur=""><span class="riskUnit"></span>
									</div>
								</div>
								<hr>
							</div>
						...
						<div class="form-group">
							<label
								class="col-sm-2 col-xs-4 control-label no-padding-right existenceCon">
							</label>
							<div class="col-sm-9 col-xs-8">
								<label class="setRiskBtn addRisk">增加级别</label> <label
									class="setRiskBtn" onclick="saveRisk()">保存</label>
							</div>
						</div>
					</form>
				</div>
...
function creatHighRisk(){
		...
		$.each(topTenX,function(i,p){
			...
			tableContent += '<tr><td style="text-align:center">'+(i+1)+'</td><td>'+p+'</td><td style="text-align:right">'+formatNumber(tempY,2)+'</td></tr>'
		});
...
　　　　　　　　　　　　//每个柱子的颜色即为colorList数组里的每一项，如果柱子数目多于colorList的长度，则柱子颜色循环使用该数组
	...
	function creatfengxian(){
		...
		var tempTable = '';
		var shunxu = [];
		var coList = ['rgb(254, 0, 0)','rgb(255, 127, 0)','rgb(255, 206, 68)','rgb(0, 192, 255)','rgb(0, 0, 254)','rgb(255, 0, 255)','rgb(110, 156, 195)'];
		var colorList  = [];
		$.each(xMsg,function(i,p){
			shunxu.push(p.substring(0,1)+"_"+i);
			if(i==xMsg.length-1){
				colorList.push('rgb(135, 184, 127)');
			}else{
				colorList.push(coList[i]);
			}
		});
		var itemStyle = {
                //通常情况下：
...
// 			dataMsg.value=tempY.toFixed(2);
			...
			tempTable+='<tr><td style="text-align:center">'+(i+1)+'</td><td>'+dataMsg.name+'-级别所属级别区间:'+fanwei[0]+"-"+fanwei[1]+'</td><td>'+formatNumber(tempY,0);+'</td></tr>';
		});
...
function saveRisk(){
	...
	$("#loadingState").show();
	$.ajax({
		url:"${ctxPath}/v1/html/analyse/result/customFxjb.do",
		dataType:"json",
		type:"post",
		data:{
			rwh:rwh,
