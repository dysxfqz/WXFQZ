			...
			<tbody>
				<c:forEach items="${ycjgList}" var="bean" varStatus='status'>
					<tr>
						...
						<td align="center">
							<c:if test="${bean.jd == 3&&bean.zt==2}"><a href="${ctxPath}/v1/html/modelBuild/modelResult/taxpayerList.do?TASKID=${bean.rwh}" title="结果列表">结果列表</a></c:if>
							<c:if test="${bean.jd == 3&&bean.zt==2}"><a href="${ctxPath}/v1/html/modelBuild/modelResult/detail.do?TASKID=${bean.rwh}" title="结果概览">结果概览</a></c:if>
