...
<body>
	...
	<a href="${ctxPath}/v1/html/modelBuild/newMission/add.do" class=" btn btn-app btn-success newTaxBtn" style="margin-bottom:10px;">新建任务</a>
			...
			<tbody>
				<c:forEach items="${ycrwList}" var="bean" varStatus='status'>
					<tr>
						...
						<td style="text-align:center"><a href="javascript:editTask('${bean.rwh}')">编辑</a>
						<c:if test="${(bean.zt == 0 && bean.jd == 0) || (bean.zt == 0 && bean.jd == 2)}">
						<a href="javascript:void(0)" onclick="deleteTask(this,'${bean.rwh}')" style="margin-left:8px;">删除</a>
						</c:if>
						<a href="${ctxPath}/v1/html/modelBuild/taskManage/detail.do?taskId=${bean.rwh}">详情</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
...
<script src="${ctxPath}/static/source/js/dragWindow.js"></script>
	...
	function deleteTask(el,rwh){
		...
		Showbo.Msg.confirm("您确定要删除该任务吗",function(){
			 if($(".btnfocus").val() =="确定"){
				 if((taskStatus.indexOf("待处理")>=0&&jieduan.indexOf("数据采集")>=0) || (taskStatus.indexOf("待处理")>=0&&jieduan.indexOf("训练预测")>=0)){
						$.ajax({
							url:"${ctxPath}/v1/html/modelBuild/taskManage/delete.do",
							dataType:"json",
							type:"post",
							data:{taskId:rwh},
	...
	function editTask(rwh){
		...
		resizeIsoAlert();
		$.ajax({
			url:"${ctxPath}/v1/html/modelBuild/taskManage/update.do",
			dataType:"json",
			type:"post",
			data:{taskId:rwh},
			success:function(data){
	...
	$("#editTaskBtn").click(function(){
		...
		if(taskName==""){
			...
			$.ajax({
				url:"${ctxPath}/v1/html/modelBuild/taskManage/confirmUpdate.do",
				dataType:"json",
				type:"post",
				data:{
					taskId:rwh,
